    ...
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton( 
              onPressed: productState.value == null 
                ? null 
                : () async{
                  final photoPath = await CameraGalleryServiceImpl().selectPhoto();
                  if(photoPath == null) return;
                  ref.read(productFormProvider(productState.value!.product!).notifier).updateProductImage(photoPath);
                },
              icon: const Icon(Icons.photo_library_outlined)
            ),
            IconButton(
              onPressed:
                productState.value == null ? null :
              () async{
